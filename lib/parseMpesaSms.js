...
function extractCode(text) {
  // The M-Pesa transaction code always sits directly before "Confirmed",
  // but what comes before the code itself varies by forwarding app:
  // "MPESA UGO9VBTG1F Confirmed...", "From : MPESA UGO9VBTG1F Confirmed...",
  // or just "UGO9VBTG1F Confirmed..." with no MPESA/From prefix at all.
  // Matching directly against "Confirmed" handles all of these.
  const m = text.match(/([A-Z0-9]{8,12})\s+Confirmed/i);
...
const PATTERNS = [
  {
    type: 'received',
    direction: 'income',
    re: /Confirmed\.?\s*You have received Ksh([\d,]+\.\d{2}) from ([A-Za-z0-9 \-]+?)\s+([\d*]{6,12})\s+on/i,
    parse: (m, text) => ({
  ...
  },
  {
    ...
    direction: 'income',
    re: /Confirmed\.?\s*You have received Ksh([\d,]+\.\d{2}) from ([A-Za-z0-9 \-]+?)\s+([\d*]{6,12})\s+for account\s*([A-Z0-9\- ]*)\s+on/i,
    parse: (m) => ({
      amount: cleanAmount(m[1]),
      counterpartyName: m[2].trim(),
      counterpartyPhone: m[3],
  ...
  },
  {
    type: 'paybill',
    direction: 'expense',
    // Must come before the generic 'sent' pattern below — paybill account
    // numbers can be as short as 6 digits, which the 'sent' pattern's
    // phone-number group would otherwise swallow first.
    re: /Confirmed\.?\s*Ksh([\d,]+\.\d{2}) (?:sent to|paid to) ([A-Za-z0-9 &.\-]+?) for account\s*([A-Z0-9\- ]*)\s+on/i,
    parse: (m) => ({
      amount: cleanAmount(m[1]),
      counterpartyName: m[2].trim(),
      accountRef: m[3].trim(),
  ...
  },
  {
    type: 'sent',
    direction: 'expense',
    re: /Confirmed\.?\s*Ksh([\d,]+\.\d{2}) sent to ([A-Za-z0-9 \-]+?)\s+([\d*]{6,12})\s+on/i,
    parse: (m) => ({
      amount: cleanAmount(m[1]),
      counterpartyName: m[2].trim(),
      counterpartyPhone: m[3],
