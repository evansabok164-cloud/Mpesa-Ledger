...
const PATTERNS = [
  {
    type: 'received',
    direction: 'income',
    re: /Confirmed\.?\s*You have received Ksh([\d,]+\.\d{2}) from ([A-Z0-9 ]+?)\s+(\d{9,12})\s+on/i,
    parse: (m, text) => ({
  ...
  },
  {
    ...
    direction: 'income',
    re: /Confirmed\.?\s*You have received Ksh([\d,]+\.\d{2}) from ([A-Z0-9 ]+?)\s+(\d{9,12})\s+for account\s*([A-Z0-9\- ]*)\s+on/i,
    parse: (m) => ({
      amount: cleanAmount(m[1]),
      counterpartyName: m[2].trim(),
      counterpartyPhone: m[3],
